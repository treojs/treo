 ...
 *
 * @param {String} name
 * @param {Object} opts { transaction: false }
 * @return {Store}
 */

Treo.prototype.store = function(name, opts) {
  if (opts && opts.transaction) this.stores[name].setTransaction(opts.transaction);
...
Treo.prototype.transaction = function(type, stores, fn, cb) {
  this.getInstance(function(err, db) {
    if (err) return cb(err);
    var tr = db.transaction(stores, type);
    tr.onerror = tr.onabort = cb;
    fn(tr);
    tr.oncomplete = function oncomplete() {
      if (tr.hasComplete && cb) cb();
    };
  });
};
