...
Store.prototype.put = function(key, val, cb) {
  ...
  this.db.transaction([name], 'write', function(err, tr) {
...
Store.prototype.get = function(key, cb) {
  var name = this.name;
  this.db.transaction([name], 'read', function(err, tr) {
    if (err) return cb(err);
    var objectStore = tr.objectStore(name);
 ...
 * Count.
 *
 * @param {Any|IDBKeyRange} [range]
 * @param {Function} cb
 */

Store.prototype.count = function(range, cb) {
  if (type(range) == 'function') {
    cb = range;
    range = null;
  }
  var name = this.name;
  this.db.transaction([name], 'read', function(err, tr) {
    if (err) return cb(err);
    var objectStore = tr.objectStore(name);
    var req = range ? objectStore.count(parseRange(range)) : objectStore.count();
    req.onerror = cb;
    req.onsuccess = function onsuccess(e) { cb(null, e.target.result) };
  });
};
...
Store.prototype.all = function(range, cb) {
  if (type(range) == 'function') {
    cb = range;
    range = null;
  }
  ...
  this.cursor({ iterator: iterator, range: range }, function(err) {
...
Store.prototype.cursor = function(opts, cb) {
  ...
  this.db.transaction([name], 'read', function(err, tr) {
    ...
    var store = opts.index
      ...
      : tr.objectStore(name);
    var req = opts.range
      ? store.openCursor(parseRange(opts.range))
      : store.openCursor();
