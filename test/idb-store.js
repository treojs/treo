...
describe('Store', function() {
  ...
  it('#cursor', function() {
    ...
    return Promise.all([
      ...
      magazines.cursor({ direction: 'prev', iterator: iterator(2) }).then(function() {
        expect(pluck(results[2], 'id')).eql(['id4', 'id3', 'id2', 'id1']);
      }),
      magazines.cursor({ range: { lte: 'id2' }, iterator: iterator(3) }).then(function() {
        expect(pluck(results[3], 'id')).eql(['id1', 'id2']);
      }),
      magazines.cursor({ range: { lte: 'id3' }, direction: 'prev', iterator: iterator(4) }).then(function() {
